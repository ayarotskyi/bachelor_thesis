...
def create_activation_animation(model_path, csv_path, images_dir):
    ...
    timestamp_array = csv_data[:, 2]

    # Prepare animation data
    animation_data = []

    for index in tqdm(range(0, 254)):
        image_memory_stack = np.zeros((4, 100, 400))
        stack_size = 0
        current_index = index
        next_timestamp = None
        while stack_size < 4:
            ...
            if current_timestamp == 0:
                break
            current_index -= 1
        image_memory_stack[4 - stack_size:] = image_memory_stack[4 - stack_size:][::-1]
        ...
        input_image = combined_image.reshape(1, 4, 100, 400, 1)

        # Create activation model
        layer_names = ['conv_lstm1', 'conv_lstm2', 'conv_lstm3', 'conv_lstm4', 'conv_lstm5']
        ...
        # for i in range(5):
        #     print(activations[i].shape, np.mean(np.concatenate(activations[i][0]), axis=-1).shape)
        results = model.predict(input_image, verbose=0)

        animation_data.append({
            'original_image': original_image, 
            'memory_stack': np.concatenate(combined_image), 
    ...
    for i in range(5):
        ...
        avg_activation = np.mean(np.concatenate(animation_data[0]['activations'][i][0]), axis=-1) if i < 4 else np.mean(animation_data[0]['activations'][i][0], axis=-1)
    ...
    def update(frame):
        ...
        # Update activation layers
        for i in range(5):
            avg_activation = np.mean(np.concatenate(animation_data[frame]['activations'][i][0]), axis=-1) if i < 4 else np.mean(animation_data[frame]['activations'][i][0], axis=-1)
            im_activations[i].set_array(avg_activation)
