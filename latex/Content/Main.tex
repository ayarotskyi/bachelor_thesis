...
Data augmentation is a well-established strategy for improving model generalization, especially in cases where collecting large, diverse datasets is infeasible. In the context of time series data, augmentation methods can be particularly impactful due to the sequential dependencies and temporal structures inherent to the data. A comprehensive overview and taxonomy of such techniques are presented in the work by Iglesias et al. \autocite{iglesias2023data}, which categorizes time series augmentation strategies into domain-specific and domain-agnostic methods, including time warping, jittering, permutation, and window slicing, among others.

This work adopts a hybrid augmentation strategy that applies both temporal and visual transformations to the training data. For the temporal component, **time warping** is implemented by dynamically adjusting the maximum allowed frame frequency \( f_{\text{max}} \) for data pairs retrieved from the augmentation buffer. By increasing or decreasing the temporal spacing between frames in a sequence, the generator effectively simulates variations in system response time or actuation speed, thus improving the model's robustness to timing deviations.

On the visual side, several spatial transformations are applied to the input images. These transformations are applied probabilistically and independently to each image in the memory stack:

\begin{itemize}
  \item \textbf{Gaussian Noise Injection:} Gaussian noise with a random standard deviation \( \sigma \in [0, 0.5] \) is added to simulate sensor noise or varying lighting conditions.
...
By combining time series augmentation principles, as outlined in \autocite{iglesias2023data}, with low-level image manipulation techniques, the data generator enhances both temporal and perceptual diversity in the training dataset. This dual-augmentation approach has shown to significantly improve the stability and adaptability of the resulting control model, particularly in environments with noisy inputs or fluctuating timing characteristics.
...
% TODO: check augmentation, create illustrations for the augmentation and finish hyperparameters
