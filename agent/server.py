...
run_data = np.array(
    [], dtype=[("x", np.float64), ("y", np.float64), ("timestamp", np.float64)]
)
...
def receive_data(process_pool, conn: socket.socket):
    ...
    while True:
        ...
        process_pool.apply_async(
            image_saver_handle, args=(directory, frame_data, index)
        )
        index += 1

        # Update data array
        run_data = np.append(
            run_data, np.array([(x, y, timestamp)], dtype=run_data.dtype)
        )
        ...
        cv2.line(
            frame,
            (start_x, start_y),
            (start_x - scaled_x, start_y - scaled_y),
            (0, 255, 0),
            2,
        )
