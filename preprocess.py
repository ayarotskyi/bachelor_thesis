...
def edge_detection_pipeline(image_path):
    ...
    canny_edges = cv2.Canny(
        enhanced, threshold1=lower_threshold, threshold2=upper_threshold, apertureSize=5
    )
    ...
    log_normalized = cv2.normalize(
        np.abs(log_edges), None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U
    )
    ...
    results = {
        ...
        "original_uncut": cv2.resize(
            cv2.imread(image_path, cv2.IMREAD_GRAYSCALE), (400, 200)
        ),
    }
...
if __name__ == "__main__":
    ...
    try:
        ...
        image_path = "reduced_data/images/0.png"
